...
const createAccount = asyncHandler(async (req, res) => {
  ...
  return new ApiResponse(201, 'Account created successfully', account).send(res);
});
...
const freezeAccount = asyncHandler(async (req, res) => {
  const account = await accountService.setAccountStatus(req.params.id, 'FROZEN');
  return new ApiResponse(200, 'Account frozen', account).send(res);
});

const unfreezeAccount = asyncHandler(async (req, res) => {
  const account = await accountService.setAccountStatus(req.params.id, 'ACTIVE');
