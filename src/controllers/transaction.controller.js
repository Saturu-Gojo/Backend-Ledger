...
const getHistory = asyncHandler(async (req, res) => {
  ...
  const result = await transactionService.getHistoryForAccount(account._id, { page, limit });
  return new ApiResponse(200, 'Transaction history fetched', result).send(res);
});

const getByReference = asyncHandler(async (req, res) => {
  const transaction = await transactionService.getByReference(req.params.reference);
